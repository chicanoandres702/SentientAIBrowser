...
export type VerifyResult = 'verified' | 'unverified';
...
export async function verifyAction(
    ...
    if (PASSTHROUGH.has(actionType)) return 'verified';
    if (INPUT_ACTIONS.has(actionType)) return 'verified';
    try {
        // Why: brief settle time so browser finishes rendering before we diff
        await page.waitForTimeout(500);
        const urlAfter = page.url();
        if (urlAfter !== urlBefore) return 'verified';
        // Lightweight ARIA diff — use ariaSnapshot (same as getAriaSnapshot) to detect DOM mutations
        const snapAfter = await (page as any).ariaSnapshot().catch(() => '');
        if (snapAfter && snapAfter !== snapBefore) return 'verified';
        // NAV actions with no observable change — flag for retry
        if (NAV_ACTIONS.has(actionType)) return 'unverified';
        return 'verified';
    } catch {
        return 'unverified';
    }
...
export function isCaptchaPage(url: string): boolean {
...
export async function trySolveCaptcha(page: Page): Promise<boolean> {
    if (!isCaptchaPage(page.url())) return false;
    try {
        // iframe-hosted captcha (reCaptcha / hCaptcha)
        for (const f of page.frames()) {
            if (/recaptcha|hcaptcha/.test(f.url())) {
                await f.click('[class*="checkbox"], .recaptcha-checkbox, [id="checkbox"]',
                    { timeout: 2000 }).catch(() => {});
        ...
        await page.click('[class*="captcha-checkbox"], [id*="captcha"] button, [id*="captcha-submit"]',
            { timeout: 2000 }).catch(() => {});
        return true;
    } catch { return false; }
}
