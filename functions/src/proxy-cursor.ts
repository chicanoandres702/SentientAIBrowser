 ...
 * [Subtask] Inject a red cursor dot into the live Playwright DOM after each action
 * [Upstream] executeAriaAction -> [Downstream] page DOM + all screenshots
 ...
 */
export async function injectCursorAt(page: Page, locator: Locator): Promise<void> {
    try {
        ...
        await page.evaluate(({ x, y }: { x: number; y: number }) => {
            let el = document.getElementById('__sentient_cursor__') as HTMLDivElement | null;
            if (!el) {
    ...
    } catch { /* non-fatal — page may have navigated away */ }
}
