...
NOTE: You have been provided with both a DOM Map (logic) and a Screenshot (visual). Use the screenshot to identify elements that might be missing or overlaying in the DOM (e.g. popups, ads, captchas).`;
    const lessons = await (0, llm_memory_service_1.getLessonsLearned)(((_a = firebase_config_1.auth.currentUser) === null || _a === void 0 ? void 0 : _a.uid) || 'anonymous', prompt);
    const relevantContext = context ? await (0, knowledge_hierarchy_service_1.getRelevantContext)(((_b = firebase_config_1.auth.currentUser) === null || _b === void 0 ? void 0 : _b.uid) || 'anonymous', context) : '';
...
`;
    try {
        const model = (0, ai_1.getGenerativeModel)(firebase_config_1.ai, { model: 'gemini-2.5-flash' });
        ...
        const parsedAction = JSON.parse(cleanedText);
