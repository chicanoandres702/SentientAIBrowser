...
const planningNode = async (state) => {
    sentientLogger_1.sentientLogger.trace('[DeepResearch] Planning node: generating research plan...');
    ...
    try {
        ...
        sentientLogger_1.sentientLogger.error('[DeepResearch] Failed to parse plan JSON, using single-category fallback');
    ...
    sentientLogger_1.sentientLogger.trace(`[DeepResearch] Plan created: ${state.research_plan.length} categories`);
    return state;
};
exports.planningNode = planningNode;
const synthesisNode = async (state) => {
    sentientLogger_1.sentientLogger.trace('[DeepResearch] Synthesis node: generating final report...');
    ...
    sentientLogger_1.sentientLogger.trace('[DeepResearch] Final report saved.');
    return state;
};
