...
const determineNextAction = async (userId, prompt, domMap, screenshotBase64, domain, lookedUpDocs = [], isScholarMode = false, context, ariaSnapshot, apiKeyOverride) => {
    var _a, _b;
    if (!apiKeyOverride) {
        sentientLogger_1.sentientLogger.error('[LLM] ❌ STAGE 3 FAIL — Runtime Gemini API key required. Must be provided in request header (x-gemini-api-key). Set key in Settings > LLM OVERRIDE.');
        return null;
    }
    sentientLogger_1.sentientLogger.trace('Sending page state to LLM. Domain:', domain, 'Scholar:', isScholarMode, 'ARIA:', !!ariaSnapshot, 'DOM:', domMap.length);
...
`;
    try {
        sentientLogger_1.sentientLogger.trace(`[LLM] ✅ STAGE 3 — using runtime key (${apiKeyOverride.substring(0, 8)}...), calling gemini-2.5-flash`);
        const genAI = new generative_ai_1.GoogleGenerativeAI(apiKeyOverride);
        ...
        if (!llmResponseText) {
            sentientLogger_1.sentientLogger.error('[LLM] ❌ STAGE 3 FAIL — empty response from Gemini');
            return null;
        }
        ...
        sentientLogger_1.sentientLogger.trace(`[LLM] ✅ STAGE 3 DONE — plan: "${parsed.execution.plan}" | steps: ${parsed.execution.segments.flatMap(s => s.steps).length}`);
        return parsed;
    }
    catch (error) {
        sentientLogger_1.sentientLogger.error(`[LLM] ❌ STAGE 3 FAIL — Gemini error: ${(_a = error === null || error === void 0 ? void 0 : error.status) !== null && _a !== void 0 ? _a : ''} ${(_b = error === null || error === void 0 ? void 0 : error.message) !== null && _b !== void 0 ? _b : error}`);
        return null;
    }
};
