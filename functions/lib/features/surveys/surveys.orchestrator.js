...
class SurveyOrchestrator {
    ...
    static evaluateDashboard(domMap) {
        sentientLogger_1.sentientLogger.trace("[SurveyOrchestrator] Evaluating Dashboard DOM...");
        // 1. Map raw nodes to typed SurveyData objects
        const availableSurveys = (0, surveys_utils_1.parseSurveyNodes)(domMap);
        if (availableSurveys.length === 0) {
            sentientLogger_1.sentientLogger.trace("[SurveyOrchestrator] No valid surveys found on page.");
            return null;
        }
        ...
        sentientLogger_1.sentientLogger.trace(`[SurveyOrchestrator] Best Survey Found: ID ${bestSurvey.id} (${bestSurvey.rewardSB} SB / ${bestSurvey.timeMinutes} Min = ${bestSurvey.yieldRatio.toFixed(2)} Yield)`);
        return bestSurvey;
    }
}
