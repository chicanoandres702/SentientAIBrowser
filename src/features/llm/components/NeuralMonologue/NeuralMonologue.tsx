...
export const NeuralMonologue = () => {
    ...
    // Why: use onAuthStateChanged for a stable uid string — avoids
    // teardown/remount on every auth object reference change (which caused blinking).
    useEffect(() => {
        ...
        <Animatable.View animation="fadeInUp" duration={800} style={styles.container}>
            <LinearGradient colors={['rgba(15,10,25,0.9)', 'rgba(5,5,8,0.98)']} style={styles.gradient}>
                ...
                <View style={styles.header}>
                    {/* Why: static dot — no infinite animation to prevent UI blinking */}
