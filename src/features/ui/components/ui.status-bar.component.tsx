// Feature: UI | Source: SentientStatusBar.tsx
// Task: Migrate SentientStatusBar to feature module
...
import { styles } from './ui.status-bar.styles';
...
export const SentientStatusBar: React.FC<Props> = React.memo(
  ({
    isAIMode, statusMessage, useProxy, theme, domain, isScholarMode, totalEarnings,
  }) => {
    ...
    useEffect(() => {
      Animated.loop(
        Animated.sequence([
          Animated.timing(seekerPos, { toValue: 1, duration: 5000, useNativeDriver: false }),
          Animated.timing(seekerPos, { toValue: 0, duration: 5000, useNativeDriver: false }),
        ])
      ).start();
    }, []);
      ...
      <View style={[styles.bar, { borderTopColor: accent + '18' }]}>
        ...
        <View style={styles.left}>
          <Animatable.View
            ...
            style={[
              styles.dot,
              { backgroundColor: isActive ? accent : BASE.inactive, shadowColor: accent, shadowOpacity: isActive ? 0.9 : 0, shadowRadius: 6 },
            ]}
          />
          ...
          <Text style={[
            styles.msg,
            { color: isActive ? colors.text : colors.textMuted, textShadowColor: accent, textShadowRadius: isActive ? 8 : 0 },
          ]}>
