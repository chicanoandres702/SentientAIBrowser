...
import { collection, doc, setDoc, updateDoc, deleteDoc, onSnapshot, query, where, limit, serverTimestamp, writeBatch } from 'firebase/firestore';
import { db } from '../auth/firebase-config';
import { TabItem } from '../browser/types';
...
export const listenToTabs = (userId: string, callback: (tabs: TabItem[]) => void) => {
    // Why: no orderBy — proxy docs may not have updated_at, which would silently exclude them
    // from an ordered query. Simple where filter is sufficient for tab sync.
    const q = query(
