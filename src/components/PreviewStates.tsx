// Feature: Browser | Why: Sub-components for preview states extracted for 100-line law
import React from 'react';
import { View, Text, TouchableOpacity, ActivityIndicator } from 'react-native';
import { previewStyles as styles } from '../features/browser/components/BrowserPreview.styles';
import { PROXY_BASE } from '../features/browser/services/proxy-health.service';
...
export const PreviewLoader = ({ status, accent }: { status: string; accent: string }) => (
    <View style={styles.loader}>
        ...
        <Text style={styles.loadingText}>{status === 'checking_proxy' ? 'Checking proxy…' : 'Loading preview…'}</Text>
    </View>
);
...
export const EmptyState = ({ status, error, proxyOnline, onRetry, accent, dimAccent }: any) => (
    <View style={styles.noPreview}>
        <Text style={styles.statusTitle}>
            {status === 'snapshot_error' ? '⚠ Preview Error' : status === 'proxy_offline' ? '🔌 Proxy Offline' : 'Browser Preview'}
        </Text>
        {status === 'waiting_for_screenshot' && (
            <Text style={styles.statusHint}>Waiting for screenshot sync…{'\n'}The proxy is capturing the page.</Text>
        )}
        {status === 'proxy_offline' && (
            <Text style={styles.statusHint}>Cannot reach the proxy at{'\n'}{PROXY_BASE}{'\n\n'}Start the proxy server and tap Retry.</Text>
        )}
        {status === 'snapshot_error' && (
            <Text style={[styles.statusHint, { color: BASE.danger }]}>{error || 'Preview failed to load'}</Text>
        )}
        ...
        {status !== 'no_tab' && (
            <Text style={styles.proxyHint}>
                {proxyOnline === true ? '● Proxy online' : proxyOnline === false ? '○ Proxy unreachable' : '◌ Checking…'}
            </Text>
        )}
    </View>
);
